...
//     const artist = song.artists.primary.map(artist => artist.name).join(',');
//     const album = song.album.name;
//     const year = song.year.toString();

//     const url = `${BASE_URL}?audioUrl=${audioUrl}&imageUrl=${imageUrl}&songName=${songName}&artist=${artist}&album=${album}&year=${year}`;
//     const path = `${RNFS.DownloadDirectoryPath}/${songName.replace(/[/\\?%*:|"<>]/g, '-').replace(/\s+/g, '_')}.m4a`;
...
//     // Path: Android app-specific Downloads folder or iOS Documents folder
